...
export default async function VerifyRequest(props: { searchParams: Promise<Props["searchParams"]> }) {
    ...
    const verificationToken = await db.query.verificationTokens.findMany({
        where: (verificationToken, { eq }) => eq(verificationToken.identifier, data),
        limit: 1,
                ...
                <div className="text-center space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-sm text-muted-foreground">
                            A sign-in link has been sent to <span className="font-medium text-foreground">{data}</span>
                        </p>
                    </div>
                </div>
            </CardHeader>
