...
export function getPaginationQueryParams(searchParams: URLSearchParams) {
    ...
    const parsedPageSize = pageSizeParam ? parseInt(pageSizeParam) : DEFAULT_PAGE_SIZE;

    const page = isNaN(parsedPage) ? DEFAULT_PAGE : parsedPage;
    const pageSize = isNaN(parsedPageSize) ? DEFAULT_PAGE_SIZE : parsedPageSize > MAX_PAGE_SIZE ? MAX_PAGE_SIZE : parsedPageSize;
